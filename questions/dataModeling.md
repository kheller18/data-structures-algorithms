...
25. What are the types of visual data modeling techniques?
    + `Entity-Relationship (ER) Model`: This is a data modeling methodology that designs traditional databases and aids in performing data normalization by reducing data redundancy. It helps in database schema visualization of top-level data view. The data modeling architect identifies the following:
      + Entities and their attributes that represent objects/tables in the relational databases.
      + Relationships between the entities.
    + `UML (Unified Modeling Language)`: UML is a general-purpose language used in database modeling, development and visualization in software engineering. It consists of different diagrams for representing the software systems and one of them is Class Diagram. Class diagrams are close similar to ER diagrams and define the following:
