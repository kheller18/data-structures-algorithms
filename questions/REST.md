...
3. What is Uniform Resource Identifier (URI)?
   + `Uniform Resource Identifier (URI)`: the full form of URI which is used for identifying each resource of the REST architecture. URI is of the format:
     + `<protocol>://<service-name>/<ResourceType>/<ResourceID>`
   + There are 2 types of URI:
     ![URI](../images/rest/uri.png)
     + `Uniform Resource Name (URN)`: identifies the resource by means of a name that is both unique and persistent.
     ...
     + `Uniform Resource Locator  (URL)`: the information regarding fetching of a resource from its location.
       + Examples include:
...
5. What is the concept of statelessness in REST?
   + `Statelessness`: REST architecture is designed in such a way that the client state is not maintained on the server. The context is provided by the client to the server using which the server processes the client’s request. The session on the server is identified by the session identifier sent by the client.
   ![statelessness](../images/rest/statelessness.png)

6. What do you understand by JAX-RS?
   + `Java API for RESTful Web Services`: a Java-based specification defined by JEE for the implementation of RESTful services. The JAX-RS library makes usage of annotations from Java 5 onwards to simplify the process of web services development. The latest version is 3.0 which was released in June 2020. This specification also provides necessary support to create REST clients.
...
8. What are the HTTP Methods?
   + `HTTP Methods`: are also known as HTTP Verbs. They form a major portion of uniform interface restriction followed by the REST that specifies what action has to be followed to get the requested resource. Below are some examples of HTTP Methods:
     + `GET`: fetching details from the server and is basically a read-only operation.
     + `POST`: for the creation of new resources on the server.
     + `PUT`: to update the old/existing resource on the server or to replace the resource.
...
10. Define Messaging in terms of RESTful web services.
    + `Messaging`: the technique of sending a message from the REST client to the REST server in the form of an HTTP request and the server responding back with the response as HTTP Response. The messages contained constitute the data and the metadata about the message.
    ![messaging](../images/rest/messaging.png)

11. Differentiate between SOAP and REST?
  ![soap vs rest](../images/rest/soap_rest.png)
    ...
    + RESTful web services use REST API as means of implementation using the HTTP protocol. REST API is nothing but an application programming interface that follows REST architectural constraints such as statelessness, cacheability, maintainability, and scalability. It has become very popular among the developer community due to its simplicity. Hence, it is very important to develop safe and secure REST APIs that follow good conventions. Below are some best practices for developing REST APIs:
      ...
      + While naming the resource endpoints, ensure to use plural nouns and not verbs. The API endpoints should be clear, brief, easy to understand, and informative. Using verbs in the resource name doesn’t contribute much information because an HTTP request already has what the request is doing in its HTTP method/verb. An appropriate HTTP verb should be used to represent the task of the API endpoint.
        + Below are the most commonly used HTTP methods to define the verb:
          + `GET`: get/retrieve the resource data
          + `POST`: create new resource data
          + `PUT`: update the existing resource data
          + `DELETE`: remove the resource data
...
15. What are the differences between REST and AJAX?
  ![rest vs ajax](../images/rest/rest_ajax.png)

16. Can you tell what constitutes the core components of HTTP Request?
    + `Method/Verb`: tells what methods the request operation represents. Methods like GET, PUT, POST, DELETE, etc are some examples.
    + `URI`: This part is used for uniquely identifying the resources on the server.
    + `HTTP Version`: indicates what version of HTTP protocol you are using. An example can be HTTP v1.1.
    + `Request Header`: the details of the request metadata such as client type, the content format supported, message format, cache settings, etc.
...
17. What constitutes the core components of HTTP Response?
    + `Response Status Code`: the server response status code for the requested resource. Example- 400 represents a client-side error, 200 represents a successful response.
    + `HTTP Version`: the HTTP protocol version.
    + `Response Header`: the metadata of the response message. Data can describe what is the content length, content type, response date, what is server type, etc.
    + `Response Body`: contains what is the actual resource/message returned from the server.
    ![http response](../images/rest/http_response.png)

18. Define Addressing in terms of RESTful Web Services.
    + `Addressing`: the process of locating a single/multiple resources that are present on the server. This task is accomplished by making use of URI (Uniform Resource Identifier). The general format of URI is:
...
22. We can develop webservices using web sockets as well as REST. What are the differences between these two?
  ![rest vs websocket](../images/rest/rest_websocket_table.png)
...
25. What is Payload in terms of RESTful web services?
    + `Payload`: the data passed in the request body. It is not the same as the request parameters. The payload can be sent only in POST methods as part of the request body.
...
30. What is the difference between idempotent and safe HTTP methods?
    + `Safe methods`: those that do not change any resources internally. These methods can be cached and can be retrieved without any effects on the resource.
    + `Idempotent methods`: those methods that do not change the responses to the resources externally. They can be called multiple times without any change in the responses.
    ![idempotent](../images/rest/idempotent.png)
