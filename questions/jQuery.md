...
2. How is jQuery different from other JavaScript frameworks?
   ...
   + oes not have any dependencies on other libraries or frameworks. (Like babel).
   + not as complex as other libraries. This makes it easier to use and understand.
   + not as heavy as other frameworks, It only uses the predefined JavaScript to make web apps interactive.
...
6. What is jQuery Mobile?
   + `jQuery Mobile`: a JavaScript library that enables developers to create mobile-first applications. It's a lightweight framework that allows developers to create rich, touch-first interfaces that are optimized for touch devices.
...
7. What is the `$()` function in the jQuery library?
   + `$()`: function is used to access the properties of elements in the DOM (Document Object Model). `$()` is similar to JavaScript’s selector functions, but it is more powerful and has more options.
...
8. Explain `$(document).ready()` function?
   + `$(document).ready()`: function is a jQuery extension that allows you to immediately run code when the document is ready. It's most commonly used to initialize a new page or to load scripts or styles into the document. However, it can also be used to load a script or style into the document at any time.
...
10. Describe jQuery Connect in brief. Also, list its uses.
    + `jQuery Connect`: a JavaScript library that provides a declarative API for binding data to elements. It allows you to specify the data source, the element that should be bound to the data, and the callback function that will be invoked when the element is bound.
...
11. What are the jQuery functions used to provide effects?
    + Some of the methods are listed below which provide the effect:
      + `toggle()`: used to check the visibility of selected elements to toggle between hide() and show() for the selected elements where:
          + `show()`: run when the element is hidden.
          + `hide()`: run when the element is visible.
          + Syntax: `$(selector).toggle(speed, easing, callback)`
      + `slideDown()`: used to either check the visibility of selected elements or to show the hidden elements. We can use this function on the following types of hidden elements:
      ...
      + `fadeOut()`: used to change the level of opacity for element of choice from visible to hidden. When used, the fadded element will not occupy any space in DOM.
        + Syntax: `$(selector).fadeOut( speed, easing, callback )`
      + `fadeToggle()`: used for toggling between the `fadeIn()` and `fadeOut()` methods.
      ...
      + `animate()`: performs custom animation of a set of CSS properties. This method changes an element from one state to another with CSS styles.
...
12. What is the use of `css()` method in jQuery?
    + `css()`: method is used to change the style property of the selected element.

13. Which jQuery method is used to hide selected elements?
    + `hide()`: function used to try and hide the chosen element.
      + `Example`: Suppose we need to hide a division tag, that consists of `id = “ib”`. Then the jQuery code will be - `$(“#ib”).hide()`;

14. What are events in jQuery?
    + `Events`: User actions on a webpage and handling responses to those is called event handling. jQuery provides simple methods for attaching event handlers to selected elements. When an event occurs, the provided function is executed.

15. How to iterate/loop through all p elements in jQuery.
    + `$.each()`: an object iterator tool provided by jQuery, as is `.each()`, a jQuery collection `iterator()`. So for traversing all the p elements, the sample code is
      ```
      $("p").each(function() {
...
16. What is the significance of jQuery.length?
    + `jQuery.length`: property used to count the number of the elements of the jQuery object.

17. What is jQuery `click()` event?
    + `click()`: happens when we click on an HTML element.
...
18. What is the purpose of jQuery's `delay()` method? Can you use this for different types of browsers like (Internet Explorer)?
    + `delay()`: used to set the delay between two events, such as a click or a change in state. The delay can be set to a number of different values, including milliseconds, seconds, and even hours. This can be useful when you want to wait for an event to occur before doing something else.
...
19. Can you explain about `ajaxStart()` functions?
    + `ajaxStart()`: a global event that occurs when an Ajax request begins, assuming no other Ajax requests are presently active. The `ajaxStart()` event can also be used to cancel a previously called Ajax callback or to start a new one.
...
20. Can you explain about `ajaxComplete()` function?
    + `ajaxComplete()`: called regardless of whether the request is successful or fails, and a complete callback is returned, even for synchronous queries.
...
22. Can you tell something about jQuery `each()` method?
    + `each()`: allows us to loop through different datasets such as arrays or objects (even DOM objects).
...
23. What is Content Delivery Network (CDN)?
    + `Content Delivery Network (CDN)`: a big distributed system of servers spread throughout the internet in many data centres. It downloads content from servers at a greater bandwidth, resulting in speedier loading times. Several firms offer free public CDNs, including Google, Amazon, Microsoft, Yahoo, etc.

24. What is the difference between JavaScript and jQuery?
    + `JavaScript`: an interpreted language written in C and is a combination of ECMAScript and DOM.
    + `jQuery`: is a JavaScript library developed to run things faster and make things simplified for JavaScript. jQuery doesn't have the ECMAScript.
...
27. What is the purpose of jQuery's `serialize()` method?
    + `serialize()`: a utility method of the jQuery library that allows you to serialize data from a DOM element and return it in a format that can be used by other libraries.
...
28. What are the selectors in jQuery? How many types of selectors in jQuery?
    + In order to work with any element on the web page, we would first need to find it. Selectors find the HTML elements in jQuery. Some of the most commonly used and basic selectors are:
      + `Name`: select all elements which match the given element Name.
      + `#ID`: select a single element which matches with the given ID
      + `.Class`: select all elements which match with the given Class.
      + `Universal (*)`: select all elements available in a DOM.
      +` Multiple Elements E, F, G`: select the combined results of all the specified selectors E, F or G.
      + `Attribute Selector`: select elements based on their attribute value.

29. Explain how CSS classes can be manipulated in HTML using jQuery.
    + jQuery provides several methods to manipulate the CSS classes assigned to HTML elements. The most important methods are `addClass()`, `removeClass()` and `toggleClass()`.
      ...
      + `removeClass()`: remove the classes from the elements by using this method.
        + Syntax:
      ...
      + `toggleClass()`: used for adding or removing one or more classes from the selected elements in such a way that if the selected element already has the class, then it is removed. Else if an element does not have the specified class, then it is added i.e. it toggles the application of classes.
...
31. Explain jQuery no-conflict.
    + `jQuery no-conflict`: option that allows you to avoid conflicts between various JavaScript frameworks or libraries. When you utilize jQuery's no-conflict mode, you substitute the `$` variable with a new variable and assign jQuery to other JavaScript libraries. In addition, the $ symbol is utilized as a function or variable name in jQuery.
...
32. In jQuery, distinguish between the `bind()`, `live()`, and `delegate()` functions.
    + `bind()`: function does not connect events to items added after loading the DOM. In contrast, the `live()` and `delegate()` methods also attach events to future items.
...
33. Explain the operation of the jQuery `connect()` method.
    + `connect()`: a utility function that allows you to create a new jQuery object and pass it to the jQuery object's constructor. This allows you to create a new object with the same functionality as the original object, but without having to manually set up the object's properties. For example, you can pass in an existing jQuery object and it will be automatically set up to use the same properties as the original object.
...
34. Distinguish between `jQuery.min.js` and `jQuery.js`.
    + The functionality of `jQuery.min.js` and `jQuery.js` is the same. However, If you are serving a page that contains a lot of JavaScript, it is a good idea to minify it.
...
35. Explain the benefits of using a CDN for the jQuery files.
    + `Content Delivery Network (CDN)`: Hosting jQuery improves availability and performance at a reduced cost and with less network traffic, improves latency (the time it takes to send and receive a data packet from the server), and provides a device-specific version of the contents.
...
37. Can you describe the various methods for extracting a query text using regular expressions?
    + `String-based approach`: aids in the generation of Strings by comparing similar regular expressions using the.replace() function.
...
48.  Can you explain the difference between `jQuery.get()` and `jQuery.ajax()`?
     ...
     + `jQuery.get()`: uses `jQuery.ajax()` underneath to create an AJAX request typically meant for simple retrieval of information.
...
53. Explain the `.promise()` method in jQuery.
    + `.promise()`: method returns a dynamically generated promise that is resolved when all actions of a certain type bound to the collection, queued or not, have ended.
    + The method takes two optional arguments:
      + `type`: default type is “fx” which indicates that the returned promise is resolved only when all animations of the selected elements have been completed.
...
57. Can you tell the difference between `prop()` and `attr()`s?
    ...
    + `attr()`: gives the default value of a property
    + `prop()`: returns its current value.
